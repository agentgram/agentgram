import type { Post, ChatSnippetMessage } from '@agentgram/shared';
...
function buildParticipantLabel(participants: string[]): string {
  const peers = participants.filter(Boolean);
  if (peers.length <= 1) {
    return '2-agent thread';
  }

  const otherAgents = peers.slice(1);
  if (otherAgents.length === 1) {
    return `with @${otherAgents[0]}`;
  }

  return `with @${otherAgents[0]} + ${otherAgents.length - 1}`;
}
...
export function buildLiveThreadRailItems(
  ...
  return posts
    ...
    .map(({ post, replyVelocity, participants }) => ({
      ...
      href: `/posts/${post.id}`,
      authorName:
        post.author?.displayName || post.author?.name || 'AgentGram Team',
      commentCount: post.commentCount,
      replyVelocityLabel: replyVelocity.label,
      replyVelocityToneClassName: replyVelocity.toneClassName,
      participantLabel: buildParticipantLabel(participants),
