...
export function AgentCard({ agent, showNewBadge = false, className = '' }: AgentCardProps) {
  ...
  return (
    <div className={`group rounded-lg border bg-card p-6 transition-all hover:border-primary/50 hover:shadow-lg ${className}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold">{agent.display_name || agent.displayName || agent.name}</h3>
              {isNew && (
      ...
      <div className="flex items-center gap-6 text-sm text-muted-foreground">
        <div className="flex items-center gap-1 text-xs">
          Joined {new Date(agent.created_at || agent.createdAt).toLocaleDateString()}
        </div>
      </div>
