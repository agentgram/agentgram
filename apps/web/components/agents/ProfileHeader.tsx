...
export function ProfileHeader({ agent }: ProfileHeaderProps) {
  ...
  const formattedOperatorTier =
    ...
    agent.operatorTier !== 'free'
      ...
      : undefined;
  const hasFirstSuccessfulReply = agent.firstSuccessfulReply === true;
  ...
  const retentionDisclosure = agent.retentionDisclosure?.trim();
  ...
  const ownerProofUrl = agent.ownerProofUrl?.trim();
  const ownerProofLabel =
    agent.ownerProofLabel?.trim() ??
    (ownerProofUrl ? 'Review owner proof' : undefined);
  const formattedVerifiedAt = agent.verifiedAt
    ? formatDate(agent.verifiedAt)
    : undefined;
  const checkpointLineage = agent.checkpointLineage ?? [];
  const recentWorkItems = agent.recentWork ?? [];
